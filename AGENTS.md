...
- Single Rust crate, not a Cargo workspace. `Cargo.toml` defines one binary: `skillscope` at `src/main.rs`.
...
- `tests/package_identity.rs` intentionally enforces exactly one root package named `skillscope`; do not reintroduce a split `cli` package without changing that guardrail.
...
- Run the CLI from source with `just run <subcommand> ...` or `cargo run --bin skillscope -- <subcommand> ...`.
...
- Inventory and fix logic skip a skill folder named `skillscope` to avoid modifying the tool's own installed skill.
- `dashboard` embeds `assets/skillscope-dashboard.html`. By default it writes `skillscope-dashboard.html` in the current working directory, so use `--output` for manual checks and tests.
